...
use regex::Regex;
use lucid_suggest_core::{Store, Record, tokenize_query, tokenize_record, SearchResult};
    ...
    STORE.with(|cell| {
        if cell.borrow().is_none() {
            ...
            for val in parsed.as_array().unwrap() {
                store.add(Record {
                    ix:     0,
                    id:     val["id"].as_u64().unwrap() as usize,
                    title:  tokenize_record(val["title"].as_str().unwrap(), &lang),
                    rating: val["rating"].as_u64().unwrap() as usize,
                })
            }
...
fn ecommerce_joined_match() {
    using_store(|store| {
        let query = tokenize_query("night light", &lang_english());
...
fn ecommerce_unpopular_hit() {
    using_store(|store| {
        let query = tokenize_query("wise", &lang_english());
        let hits  = store.search(&query.to_ref());
...
fn ecommerce_longest_match() {
    using_store(|store| {
        let query = tokenize_query("i wise", &lang_english());
        let hits  = store.search(&query.to_ref());
        assert_hit_match(&hits[0], r"<wise>");
...
fn ecommerce_match_surface() {
    using_store(|store| {
        let query = tokenize_query("it it ornament", &lang_english());
        let hits  = store.search(&query.to_ref());
...
fn ecommerce_case__little_bird() {
    using_store(|store| {
        let query = tokenize_query("little bird", &lang_english());
        let hits  = store.search(&query.to_ref());
...
fn ecommerce_case__50s() {
    using_store(|store| {
        let query = tokenize_query("50s", &lang_english());
        let hits  = store.search(&query.to_ref());
...
fn ecommerce_case__hold() {
    using_store(|store| {
        let query = tokenize_query("hold", &lang_english());
        let hits  = store.search(&query.to_ref());
...
fn ecommerce_case__backp() {
    using_store(|store| {
        let query = tokenize_query("backp", &lang_english());
        let hits  = store.search(&query.to_ref());
...
fn ecommerce_case__chop() {
    using_store(|store| {
        let query = tokenize_query("chop", &lang_english());
        let hits  = store.search(&query.to_ref());
...
fn ecommerce_case__pop_corn() {
    using_store(|store| {
        let query = tokenize_query("pop corn", &lang_english());
        let hits  = store.search(&query.to_ref());
...
fn ecommerce_case__orna() {
    using_store(|store| {
        ...
        for len in 3 .. query.chars().count() {
            ...
            let query = tokenize_query(&query, &lang_english());
            let hits  = store.search(&query.to_ref());
...
fn ecommerce_case__vnt() {
    using_store(|store| {
        let query = tokenize_query("vn", &lang_english());
        let hits  = store.search(&query.to_ref());
        assert_hit_match(&hits[0], r"<vin>");

        let query = tokenize_query("vnt", &lang_english());
        let hits  = store.search(&query.to_ref());
...
//     using_store(|store| {
//         let query = tokenize_query("greenpink", &lang_english());
//         let hits  = store.search(&query.to_ref());
//         dbg!(&hits);
//         // assert_hit_match(&hits[0], r"<vin>");
//     });
...
//     using_store(|store| {
//         let query = tokenize_query("babule", &lang_english());
//         let hits  = store.search(&query.to_ref());
//         dbg!(&hits);
